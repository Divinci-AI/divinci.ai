...
**Then we measured it on the board.** On 26 September we ran the same retriever with Jev's step removed, with the same candidates interleaved instead, as a separate row. It scored **0.817** (0.800 to 0.825), against 0.947 with Jev: about thirteen points, under the same judge, with no overlap. Without Jev the row falls from first to fifth, below Qdrant and Vectorize. The right tile came first for 20 of 60 questions instead of 46. Answers scored zero rose from one or two per run to six to nine. So the board confirms what the retrieval measurement suggested: the row's lead is Jev's.
