...
SLUG = "www-rag-making-the-open-web-chattable"
...
PROMPT = (
    ...
    "depicts a graceful brass mechanical spider — an automaton web-weaver — "
    "spinning a luminous, orderly web whose strands connect dozens of small "
    "illustrated website-pages arranged like a constellation across the sheet: "
    "a philosophy codex, an observatory dome, a law library column, a rocket, "
    "a marketplace stall, a stack of manuscripts. Each connected page sits "
    "beside its own tiny corked glass vial of glowing golden thread — one "
    "small isolated vessel per site, shelved in a neat apothecary row along "
    "the bottom edge, annotated 'one index per website'. "
    "Marginal sketches celebrate the machinery: (upper left) a small open "
    "writing-desk workshop with a single humble oil lamp powering the whole "
    "loom, annotated 'built from one desk'; (upper right) an astrolabe-like "
    "compass rose whose needle points at a few chosen constellation-pages, "
    "annotated 'the router chooses in microseconds'; (lower right) two "
    "identical ornate keys — one small and plain on a workbench, one grand "
    "and mounted among clouds — shown fitting the same lock, annotated 'the "
    "same key, at home and in the cloud'. "
    "A small stamped seal at the bottom reads 'DIVINCI · WWW-RAG'. Warm cream "
    "parchment background, deep sepia and rust ink, gold-leaf highlights on "
    "the web strands and the twin keys. Crafted, scholarly, optimistic "
    "Renaissance notebook aesthetic — the same visual family as the existing "
    "RAG-Arena and Headroom hero illustrations. No modern UI elements, no AI "
    "gradient slop. 16:9 aspect ratio, wide composition."
)
