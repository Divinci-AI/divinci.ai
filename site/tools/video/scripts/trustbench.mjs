 ...
 * Flux reads a bare "id" as the English word (like Freud's), not as letters.
 * The same rule is already applied to n p m, M I T and t b p prod zero zero two.
 *
...
export default {
  ...
  lines: [
    ...
    { at: 65.7, voice: MAEVE, text: 'Every row carries the key that signed it, and a link straight to the manifest. Which is the part worth doing yourself.' },

    // ── Scene 3 — running one ────────────────────────────────────────────
    { at: 74.0, voice: MILES, text: 'Running one takes a model I D and a button, and the catalog tells you the cost before you start. About sixty-six seconds and eleven cents for this one.' },
    ...
    { at: 134.9, voice: MAEVE, text: 'Verified. Signature valid, signed by t b p prod zero zero two. Score provenance measured, model invoked, no warnings.' },
    ...
    { at: 154.0, voice: MAEVE, text: 'Swap the model I D. Fails. Inflate the sample count from eleven to five hundred. Fails. Alter the hash of the outputs. Fails.' },
    ...
    { at: 170.5, voice: MAEVE, text: 'And rotation appends rather than replaces, so a manifest signed under a retired key still verifies years later.' },

    // ── Scene 5 — attested failure ───────────────────────────────────────
    { at: 178.7, voice: MILES, text: 'A run that fails is still a first-class record. This one targeted a model I D that does not exist.' },
