...
export default {
  ...
  lines: [
    ...
    { at: 8.3, voice: MILES, text: 'Every run produces a manifest signed with a platform key. The score you are shown is provably the score that was measured.' },
    ...
    { at: 17.4, voice: MAEVE, text: 'Five boards are published right now. Four platform benchmarks, and one scored suite promoted from a workspace.' },
    ...
    // TrustBench integration is not visible there. Searching "grounding"
    // returns only Releases / Setup / AI Config / Release Settings, and
    // searching a model id returns nothing at all — no Benchmarks section, no
    // Runs section. The brief anticipated exactly this and said to cut rather
    // than narrate a feature the build does not have.
    //
