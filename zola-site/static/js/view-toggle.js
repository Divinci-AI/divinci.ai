...
(function() {
    ...
    function initViewToggle() {
        ...
        function updateContentVisibility(isCompanyView) {
            ...
            // Handle header elements separately to prevent glitching
            const headerConsumerElements = document.querySelectorAll('.nav-title .consumer-view-content');
            const headerCompanyElements = document.querySelectorAll('.nav-title .company-view-content');

            // Update header elements with opacity only (no display changes)
            headerConsumerElements.forEach(element => {
                element.style.opacity = isCompanyView ? '0' : '1';
            });

            headerCompanyElements.forEach(element => {
                element.style.opacity = isCompanyView ? '1' : '0';
            });

            // Update non-header content with display changes
            const nonHeaderConsumerElements = Array.from(consumerElements).filter(el => !el.closest('.nav-title'));
            const nonHeaderCompanyElements = Array.from(companyElements).filter(el => !el.closest('.nav-title'));

            nonHeaderConsumerElements.forEach(element => {
                if (isCompanyView) {
            ...
            nonHeaderCompanyElements.forEach(element => {
                if (isCompanyView) {
                    element.style.display = '';
                    setTimeout(() => {
                        element.style.opacity = '1';
        ...
        if (!document.getElementById('view-toggle-styles')) {
            ...
            styleEl.textContent = `
                ...
                /* Prevent header glitching with proper positioning */
                .nav-title {
                    position: relative;
                    min-width: 320px; /* Reserve space for longer company title */
                }

                .nav-title .consumer-view-content,
                .nav-title .company-view-content {
                    position: absolute;
                    left: 0;
                    top: 0;
                    width: 100%;
                ...
                /* Create invisible spacer to maintain layout */
                .nav-title::after {
                    content: "Divinci ™ - Enterprise AI Solutions";
                    visibility: hidden;
                    pointer-events: none;
                    height: 0;
                    overflow: hidden;
